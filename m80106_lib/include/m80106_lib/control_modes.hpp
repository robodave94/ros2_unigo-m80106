...
{
    float T   = 0.0f;   ///< Desired rotor torque   [N·m]   ±127.99
    float W   = 0.0f;   ///< Desired rotor speed    [rad/s] ±804.00
    float Pos = 0.0f;   ///< Desired rotor position [rad]   ±411774
    ...
    {
        MotorCmd c = zeroCmd();
        c.T   = clamp(T,   -MAX_TORQUE_NM,  MAX_TORQUE_NM);
        c.W   = clamp(W,   -MAX_SPEED_RADS, MAX_SPEED_RADS);
        c.Pos = clamp(Pos, -MAX_POS_RAD,    MAX_POS_RAD);
...
struct PositionPD : BaseControlMode
{
    ...
    {
        MotorCmd c = zeroCmd();
        c.Pos = clamp(target_pos_rad,  -MAX_POS_RAD,   MAX_POS_RAD);
        c.K_P = clamp(kp,               0.0f,           MAX_KP);
        c.K_W = clamp(kd,               0.0f,           MAX_KD);
        c.T   = clamp(feedforward_T,   -MAX_TORQUE_NM,  MAX_TORQUE_NM);
        c.W   = 0.0f;
        return c;
    }
};
...
struct VelocityPI : BaseControlMode
{
    ...
    float integral_limit    = 5.0f;   ///< Anti-windup clamp [N·m]
    ...
    MotorCmd compute(const MotorData & fb) override
    {
        ...
        MotorCmd c = zeroCmd();
        c.T   = clamp(torque,   -MAX_TORQUE_NM, MAX_TORQUE_NM);
...
{
    float target_torque_nm = 0.0f;  ///< Desired rotor torque [N·m] ±127.99
    ...
    {
        MotorCmd c = zeroCmd();
        c.T = clamp(target_torque_nm, -MAX_TORQUE_NM, MAX_TORQUE_NM);
        return c;
    }
};
...
struct ImpedanceControl : BaseControlMode
{
    ...
    float torque_limit = MAX_TORQUE_NM; ///< Safety torque saturation   [N·m]
...
{
    if (name == "brake")       return std::make_unique<BrakeMode>();
    if (name == "foc")         return std::make_unique<FOCPassthrough>();
    if (name == "calibration") return std::make_unique<CalibrationMode>();
    if (name == "position_pd") return std::make_unique<PositionPD>();
    if (name == "velocity_pi") return std::make_unique<VelocityPI>();
    if (name == "torque")      return std::make_unique<DirectTorque>();
    if (name == "impedance")   return std::make_unique<ImpedanceControl>();
